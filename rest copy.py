...
def dialogflowRequestHandler():
    ...
    if (intent == "Sickness"):
        ...
        if user is None:
            return {'fulfillmentText': personNotFound('sick')}
    ...
    if (intent == "Vacation"):
        ...
        if not bookingWorked:
            return {'fulfillmentText': personNotFound('vacation')}

        return {}

    return {'fulfillmentText': f'Intent {intent} unknown.'}

# dialogflow request entry point
@restService.route('rest/dialogflow/webhook', methods=['GET', 'POST'])
def dialogflowRequestEntryPoint():
    return asJsonResponse(dialogflowRequestHandler())
...
def getSick():
    ...
    if sicknesses.count() == 0:
        ...
        return asJsonResponse(sicknesses), 200, {'Content-Type': 'application/json; charset=utf-8'}
